...
const Home = () => {
  const [prefix, setPrefix] = useState<DefaultPrefixProps>(DEFAULT_PREFIX)
  const [data, setData] = useState<DefaultDataProps>(DEFAULT_DATA)
  const [link, setLink] = useState<DefaultLinkProps>(DEFAULT_LINK)
  const [social, setSocial] = useState<DefaultSocialProps>(DEFAULT_SOCIAL)
  const [skills, setSkills] = useState<LooseObject>(DEFAULT_SKILLS)
  const [support, setSupport] = useState<DefaultSupportProps>(DEFAULT_SUPPORT)

  const [restore, setRestore] = useState<string>("")
  const [generatePreview, setGeneratePreview] = useState<boolean>(false)
  const [generateMarkdown, setGenerateMarkdown] = useState<boolean>(false)
  ...
  const handleSkillsChange = (field: string) => {
    const change = { ...skills }
  ...
  const handleCheckChange = (field: string) => {
    const change = { ...data }
    change[field] = !change[field]
    setData(change)
  }
  ...
  const mergeDefaultWithNewDataSkills = (
    defaultSkills: LooseObject,
    newSkills: LooseObject
  ) =>
